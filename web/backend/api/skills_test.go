...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("SPAWNBOT_BUILTIN_SKILLS")
	if err := os.Setenv("SPAWNBOT_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(SPAWNBOT_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("SPAWNBOT_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("SPAWNBOT_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
