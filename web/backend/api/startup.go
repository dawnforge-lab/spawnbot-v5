...
const (
	autoStartEntryName = "SpawnbotLauncher"
	launchAgentLabel   = "io.spawnbot.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "spawnbot-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=Spawnbot Web",
			"Comment=Start Spawnbot Web on login",
