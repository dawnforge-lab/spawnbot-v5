...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to spawnbot",
		Args:  cobra.NoArgs,
		Example: `  spawnbot migrate
  spawnbot migrate --from openclaw
  spawnbot migrate --dry-run
  spawnbot migrate --refresh
  spawnbot migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.spawnbot)")
