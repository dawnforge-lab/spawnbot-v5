...
Você também pode executar o Spawnbot usando Docker Compose sem instalar nada localmente.
...
> [!TIP]
> **Usuários Docker**: Por padrão, o Gateway escuta em `127.0.0.1`, que não é acessível a partir do host. Se você precisar acessar os endpoints de saúde ou expor portas, defina `SPAWNBOT_GATEWAY_HOST=0.0.0.0` no seu ambiente ou atualize o `config.json`.

```bash
# 5. Verificar logs
docker compose -f docker/docker-compose.yml logs -f spawnbot-gateway
...
A imagem `launcher` inclui os três binários (`spawnbot`, `spawnbot-launcher`, `spawnbot-launcher-tui`) e inicia o console web por padrão, que fornece uma interface baseada em navegador para configuração e chat.
...
```bash
# Fazer uma pergunta
docker compose -f docker/docker-compose.yml run --rm spawnbot-agent -m "What is 2+2?"
...
> [!TIP]
> Configure sua chave de API em `~/.spawnbot/config.json`. Obtenha chaves de API: [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=Spawnbot&utm_content=Spawnbot&utm_medium=devrel&utm_source=OWO&utm_term=Spawnbot) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM). A busca na web é opcional — obtenha gratuitamente uma [API Tavily](https://tavily.com) (1000 consultas gratuitas/mês) ou [API Brave Search](https://brave.com/search/api) (2000 consultas gratuitas/mês).
...
**2. Configurar** (`~/.spawnbot/config.json`)
...
> `request_timeout` é opcional e usa segundos. Se omitido ou definido como `<= 0`, o Spawnbot usa o timeout padrão (120s).
