...
Arquivo de configuração: `~/.spawnbot/config.json`

### Variáveis de Ambiente

Você pode substituir os caminhos padrão usando variáveis de ambiente. Isso é útil para instalações portáteis, implantações em contêineres ou execução do spawnbot como serviço do sistema. Essas variáveis são independentes e controlam caminhos diferentes.

| Variável          | Descrição                                                                                                                             | Caminho Padrão              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `SPAWNBOT_CONFIG` | Substitui o caminho para o arquivo de configuração. Isso indica diretamente ao spawnbot qual `config.json` carregar, ignorando todos os outros locais. | `~/.spawnbot/config.json` |
| `SPAWNBOT_HOME`   | Substitui o diretório raiz para dados do spawnbot. Isso altera o local padrão do `workspace` e outros diretórios de dados.          | `~/.spawnbot`             |

**Exemplos:**

```bash
# Executar spawnbot usando um arquivo de configuração específico
...
# Executar spawnbot com todos os dados armazenados em /opt/spawnbot
# A configuração será carregada do padrão ~/.spawnbot/config.json
# O workspace será criado em /opt/spawnbot/workspace
...
O Spawnbot armazena dados no seu workspace configurado (padrão: `~/.spawnbot/workspace`):
...
O Spawnbot é executado em um ambiente sandbox por padrão. O agente só pode acessar arquivos e executar comandos dentro do workspace configurado.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.spawnbot/workspace` | Diretório de trabalho do agente           |
...
O guard de segurança do exec inspeciona apenas a linha de comando que o Spawnbot executa diretamente. Ele não inspeciona recursivamente processos filhos gerados por ferramentas de desenvolvimento permitidas como `make`, `go run`, `cargo`, `npm run` ou scripts de build personalizados.
...
* Execute o Spawnbot dentro de um contêiner ou VM se precisar de isolamento mais forte do que o guard integrado oferece.
...
O Spawnbot pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
* `SPAWNBOT_HEARTBEAT_ENABLED=false` para desativar
* `SPAWNBOT_HEARTBEAT_INTERVAL=60` para alterar o intervalo
...
| `volcengine` | LLM (Volcengine direto)                 | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Spawnbot&utm_content=Spawnbot&utm_medium=devrel&utm_source=OWO&utm_term=Spawnbot) |
...
> **Novidade:** Spawnbot agora usa uma abordagem **centrada no modelo**. Basta especificar o formato `vendor/model` (ex.: `zhipu/glm-4.7`) para adicionar novos providers — **sem alterações de código!**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Obter](https://www.volcengine.com/activity/codingplan?utm_campaign=Spawnbot&utm_content=Spawnbot&utm_medium=devrel&utm_source=OWO&utm_term=Spawnbot) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Somente OAuth                                                    |

#### Balanceamento de Carga

Configure múltiplos endpoints para o mesmo nome de modelo — Spawnbot fará round-robin automaticamente:
...
Spawnbot roteia providers por família de protocolo:
...
Spawnbot suporta tarefas agendadas via ferramenta `cron`.
...
As tarefas agendadas persistem após reinicializações em `~/.spawnbot/workspace/cron/`.
