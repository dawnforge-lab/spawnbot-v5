...
**Antigravity** (Google Cloud Code Assist) é um provedor de modelos de IA apoiado pelo Google que oferece acesso a modelos como Claude Opus 4.6 e Gemini através da infraestrutura de nuvem do Google. Este documento fornece um guia completo sobre como a autenticação funciona, como buscar modelos e como implementar um novo provedor no Spawnbot.
...
10. [Criando um Novo Provedor no Spawnbot](#criando-um-novo-provedor-no-spawnbot)
...
const antigravityPlugin = {
  ...
  register(api: SpawnbotPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<SpawnbotConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.25
- Base de código do Spawnbot (`pkg/providers/` e `pkg/auth/`)
...
Os perfis de autenticação são armazenados em `~/.spawnbot/auth.json`:
...
## Criando um Novo Provedor no Spawnbot

Os provedores do Spawnbot são implementados como pacotes Go em `pkg/providers/`. Para adicionar um novo provedor:
...
Se seu provedor requer OAuth ou autenticação especial, adicione um caso em `cmd/spawnbot/internal/auth/helpers.go`:
...
spawnbot auth login --provider your-provider

# Listar modelos (para Antigravity)
spawnbot auth models

# Iniciar o gateway
spawnbot gateway

# Executar um agente com um modelo específico
spawnbot agent -m "Hello" --model your-model
```
...
export SPAWNBOT_AGENTS_DEFAULTS_MODEL=your-model

# Substituir configurações do provedor
export SPAWNBOT_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
- **Arquivos Fonte:**
  ...
  - `pkg/auth/store.go` - Armazenamento de credenciais de autenticação (`~/.spawnbot/auth.json`)
  ...
  - `cmd/spawnbot/internal/auth/helpers.go` - Comandos CLI de autenticação
...
- Atualizar tokens OAuth: `spawnbot auth login --provider antigravity`
...
- Verificar o armazenamento do perfil de autenticação: `~/.spawnbot/auth.json`
- Executar novamente `spawnbot auth login --provider antigravity`
