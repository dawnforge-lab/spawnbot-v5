...
Spawnbot supports encrypting `api_key` values in `model_list` configuration entries.
...
Run `spawnbot onboard` — it prompts for your passphrase and generates the SSH key,
...
| Encrypted | `enc://<base64>` | Decrypted at startup using `SPAWNBOT_KEY_PASSPHRASE` |
...
1. The **passphrase** (`SPAWNBOT_KEY_PASSPHRASE`)
...
|----------|----------|-------------|
| `SPAWNBOT_KEY_PASSPHRASE` | Yes (for `enc://`) | Passphrase used for key derivation |
| `SPAWNBOT_SSH_KEY_PATH` | No | Path to SSH private key. If not set, auto-detects from `~/.ssh/spawnbot_ed25519.key` |

### SSH Key Auto-Detection

If `SPAWNBOT_SSH_KEY_PATH` is not set, Spawnbot looks for the spawnbot-specific key:
...
Run `spawnbot onboard` to generate it automatically.

`os.UserHomeDir()` is used for cross-platform home directory resolution (reads `USERPROFILE` on Windows, `HOME` on Unix/macOS).

> **Note:** An SSH key file is required for credential encryption. If no key is found and `SPAWNBOT_SSH_KEY_PATH` is not set, encryption/decryption will fail. Run `spawnbot onboard` to generate the key automatically.

---

## Migration

Because the only secret material is `SPAWNBOT_KEY_PASSPHRASE` and the SSH private key file, migration is straightforward:

1. Copy the config file to the new machine.
2. Set `SPAWNBOT_KEY_PASSPHRASE` to the same value.
3. Copy the SSH private key file to the same path (or set `SPAWNBOT_SSH_KEY_PATH` to its new location).
...
- **Both passphrase and SSH key are required.** The SSH key acts as a second factor — without it, encryption/decryption will fail. Run `spawnbot onboard` to generate the key if it doesn't exist.
- **The SSH key is read-only at runtime.** Spawnbot never writes to or modifies the SSH key file.
- **Plaintext keys remain supported.** Existing configs without `enc://` are unaffected.
- **The `enc://` format is versioned** via the HKDF `info` field (`spawnbot-credential-v1`), allowing future algorithm upgrades without breaking existing encrypted values.
