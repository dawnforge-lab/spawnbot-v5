...
Spawnbot's tools configuration is located in the `tools` field of `config.json`.
...
Before tool results are sent to the LLM, Spawnbot can filter sensitive values (API keys, tokens, secrets) from the output. This prevents the LLM from seeing its own credentials.
...
```bash
SPAWNBOT_TOOLS_EXEC_ENABLED=false
```
...
By default, Spawnbot blocks the following dangerous commands:
...
The exec guard only validates the top-level command sent to Spawnbot. It does **not** recursively inspect child
...
All configuration options can be overridden via environment variables with the format `SPAWNBOT_TOOLS_<SECTION>_<KEY>`:

For example:

- `SPAWNBOT_TOOLS_WEB_BRAVE_ENABLED=true`
- `SPAWNBOT_TOOLS_EXEC_ENABLED=false`
- `SPAWNBOT_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `SPAWNBOT_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `SPAWNBOT_TOOLS_MCP_ENABLED=true`
