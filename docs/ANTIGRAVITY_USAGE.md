# Using Antigravity Provider in Spawnbot

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in Spawnbot.
...
6.  **Paste it back into the terminal** where Spawnbot is waiting.

Spawnbot will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.spawnbot/config.json` or override it via the CLI:
...
*   **429 Rate Limit**: Antigravity has strict quotas. Spawnbot will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `spawnbot auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
