...
Trò chuyện với spawnbot của bạn qua Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot hoặc MaixCam
...
| **Pico**             | ⭐ Dễ              | Kênh giao thức bản địa Spawnbot                       |                                                                                                                  |
...
Spawnbot hiện lưu trữ định nghĩa lệnh trong một registry chung. Khi khởi động, Telegram sẽ tự động đăng ký các lệnh bot được hỗ trợ (ví dụ `/start`, `/help`, `/show`, `/list`) để menu lệnh và hành vi runtime luôn đồng bộ.
...
Nếu đăng ký lệnh thất bại (lỗi tạm thời mạng/API), kênh vẫn khởi động và Spawnbot thử lại đăng ký trong nền.
...
Spawnbot có thể kết nối WhatsApp theo hai cách:
...
Nếu `session_store_path` trống, phiên được lưu tại `<workspace>/whatsapp/`. Chạy `spawnbot gateway`; lần chạy đầu tiên, quét mã QR hiển thị trong terminal bằng WhatsApp → Thiết bị liên kết.
...
Spawnbot hỗ trợ kết nối với tài khoản WeChat cá nhân của bạn thông qua API chính thức Tencent iLink.
...
4. Chạy `spawnbot gateway` và mở QQ để trò chuyện với bot của bạn
...
* Cấu hình như trên và chạy `spawnbot gateway`
...
Spawnbot hỗ trợ ba loại tích hợp WeCom:
...
Spawnbot kết nối với Feishu qua chế độ WebSocket/SDK — không cần URL webhook công khai hay máy chủ callback.
...
OneBot là giao thức mở cho bot QQ. Spawnbot kết nối với bất kỳ triển khai tương thích OneBot v11 nào (ví dụ: [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) qua WebSocket.
