...
**Antigravity** (Google Cloud Code Assist) là nhà cung cấp mô hình AI được Google hỗ trợ, cung cấp quyền truy cập vào các mô hình như Claude Opus 4.6 và Gemini thông qua hạ tầng đám mây của Google. Tài liệu này cung cấp hướng dẫn đầy đủ về cách xác thực hoạt động, cách lấy danh sách mô hình và cách triển khai nhà cung cấp mới trong Spawnbot.
...
9. [Cấu hình](#cấu-hình)
10. [Tạo nhà cung cấp mới trong Spawnbot](#tạo-nhà-cung-cấp-mới-trong-spawnbot)
...
- Go ≥ 1.25
- Mã nguồn Spawnbot (`pkg/providers/` và `pkg/auth/`)
...
Hồ sơ xác thực được lưu trữ trong `~/.spawnbot/auth.json`:
...
## Tạo nhà cung cấp mới trong Spawnbot

Các nhà cung cấp Spawnbot được triển khai dưới dạng gói Go trong `pkg/providers/`. Để thêm nhà cung cấp mới:
...
Nếu nhà cung cấp của bạn yêu cầu OAuth hoặc xác thực đặc biệt, thêm case vào `cmd/spawnbot/internal/auth/helpers.go`:
...
- **File nguồn:**
  ...
  - `pkg/auth/store.go` - Lưu trữ thông tin xác thực (`~/.spawnbot/auth.json`)
  ...
  - `cmd/spawnbot/internal/auth/helpers.go` - Lệnh CLI xác thực
...
- Làm mới token OAuth: `spawnbot auth login --provider antigravity`
...
- Kiểm tra lưu trữ hồ sơ xác thực: `~/.spawnbot/auth.json`
- Chạy lại `spawnbot auth login --provider antigravity`
