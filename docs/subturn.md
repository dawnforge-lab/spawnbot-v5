...
The `SubTurn` mechanism is a core feature in Spawnbot that allows tools to spawn isolated, nested agent loops to handle complex sub-tasks.
...
SubTurns emit specific events to the Spawnbot `EventBus` for observability and debugging:
