...
設定ファイルパス: `~/.spawnbot/config.json`

### 環境変数

環境変数を使用してデフォルトパスを上書きできます。ポータブルインストール、コンテナ化デプロイ、または spawnbot をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `SPAWNBOT_CONFIG` | 設定ファイルのパスを上書きします。spawnbot がどの `config.json` を読み込むかを直接指定し、他のすべての場所を無視します。 | `~/.spawnbot/config.json` |
| `SPAWNBOT_HOME`   | spawnbot データのルートディレクトリを上書きします。`workspace` やその他のデータディレクトリのデフォルト場所を変更します。          | `~/.spawnbot`             |

**例：**

```bash
# 特定の設定ファイルで spawnbot を実行
# ワークスペースパスはその設定ファイル内から読み込まれます
SPAWNBOT_CONFIG=/etc/spawnbot/production.json spawnbot gateway

# /opt/spawnbot にすべてのデータを保存して spawnbot を実行
# 設定はデフォルトの ~/.spawnbot/config.json から読み込まれます
# ワークスペースは /opt/spawnbot/workspace に作成されます
...
Spawnbot は設定されたワークスペース（デフォルト: `~/.spawnbot/workspace`）にデータを保存します：
...
1. `~/.spawnbot/workspace/skills`（ワークスペース）
2. `~/.spawnbot/skills`（グローバル）
...
Spawnbot はデフォルトでサンドボックス環境で実行されます。Agent は設定されたワークスペース内のファイルアクセスとコマンド実行のみが可能です。
...
| オプション              | デフォルト値            | 説明                                  |
| ----------------------- | ----------------------- | ------------------------------------- |
| `workspace`             | `~/.spawnbot/workspace` | Agent の作業ディレクトリ              |
...
exec セキュリティガードは Spawnbot が直接起動するコマンドラインのみを検査します。`make`、`go run`、`cargo`、`npm run`、またはカスタムビルドスクリプトなどの開発ツールが生成する子プロセスは再帰的に検査しません。
...
* ビルトインガードより強力な分離が必要な場合は、コンテナまたは VM 内で Spawnbot を実行してください。
...
Spawnbot は定期タスクを自動実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成してください：
...
- `SPAWNBOT_HEARTBEAT_ENABLED=false` で無効化
- `SPAWNBOT_HEARTBEAT_INTERVAL=60` で間隔を変更
...
| `volcengine` | LLM（Volcengine 直接）                  | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Spawnbot&utm_content=Spawnbot&utm_medium=devrel&utm_source=OWO&utm_term=Spawnbot) |
...
> **新機能：** Spawnbot は**モデル中心**の設定アプローチを採用しました。`vendor/model` 形式（例：`zhipu/glm-4.7`）を指定するだけで新しい Provider を追加できます — **コード変更不要！**
...
| **VolcEngine (Doubao)** | `volcengine/`          | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI     | [取得](https://www.volcengine.com/activity/codingplan?utm_campaign=Spawnbot&utm_content=Spawnbot&utm_medium=devrel&utm_source=OWO&utm_term=Spawnbot) |
| **Antigravity**         | `antigravity/`         | Google Cloud                                        | Custom     | OAuth のみ                                                       |

#### ロードバランシング

同じモデル名に複数のエンドポイントを設定すると、Spawnbot が自動的にラウンドロビンします：
...
Spawnbot はプロトコルファミリーで Provider をルーティングします：
...
Spawnbot は `cron` ツールを通じて cron スタイルのスケジュールタスクをサポートします。
...
スケジュールタスクは再起動後も `~/.spawnbot/workspace/cron/` に保存されます。
