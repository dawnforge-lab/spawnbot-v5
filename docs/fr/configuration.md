...
Fichier de configuration : `~/.spawnbot/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de Spawnbot en tant que service système. Ces variables sont indépendantes et contrôlent des chemins différents.

| Variable          | Description                                                                                                                             | Chemin par défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `SPAWNBOT_CONFIG` | Remplace le chemin vers le fichier de configuration. Indique directement à Spawnbot quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.spawnbot/config.json` |
| `SPAWNBOT_HOME`   | Remplace le répertoire racine des données Spawnbot. Change l'emplacement par défaut du `workspace` et des autres répertoires de données. | `~/.spawnbot`             |
...
Spawnbot stocke les données dans votre workspace configuré (par défaut : `~/.spawnbot/workspace`) :
...
Spawnbot s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes que dans le workspace configuré.
...
| ----------------------- | ----------------------- | ------------------------------------------------- |
| `workspace`             | `~/.spawnbot/workspace` | Répertoire de travail de l'agent                  |
...
Le garde de sécurité exec n'inspecte que la ligne de commande lancée directement par Spawnbot. Il n'inspecte pas récursivement les processus enfants générés par les outils de développement autorisés tels que `make`, `go run`, `cargo`, `npm run` ou les scripts de build personnalisés.
...
* Exécutez Spawnbot dans un conteneur ou une VM si vous avez besoin d'une isolation plus forte que celle fournie par le garde intégré.
...
Spawnbot peut effectuer des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
* `SPAWNBOT_HEARTBEAT_ENABLED=false` pour désactiver
* `SPAWNBOT_HEARTBEAT_INTERVAL=60` pour changer l'intervalle
...
> **Nouveauté :** Spawnbot utilise désormais une approche **centrée sur le modèle**. Spécifiez simplement le format `vendor/model` (ex. `zhipu/glm-4.7`) pour ajouter de nouveaux providers — **aucune modification de code requise !**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Obtenir](https://www.volcengine.com/activity/codingplan?utm_campaign=Spawnbot&utm_content=Spawnbot&utm_medium=devrel&utm_source=OWO&utm_term=Spawnbot) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | OAuth uniquement                                                 |

#### Équilibrage de Charge

Configurez plusieurs endpoints pour le même nom de modèle — Spawnbot effectuera automatiquement un round-robin :
...
Spawnbot route les providers par famille de protocole :
...
Spawnbot supporte les tâches planifiées via l'outil `cron`. L'agent peut définir, lister et annuler des rappels ou tâches récurrentes.
...
Les tâches planifiées persistent après redémarrage dans `~/.spawnbot/workspace/cron/`.
