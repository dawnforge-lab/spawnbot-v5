...
10. [Créer un nouveau fournisseur dans Spawnbot](#créer-un-nouveau-fournisseur-dans-spawnbot)
...
- Go ≥ 1.25
- Base de code Spawnbot (`pkg/providers/` et `pkg/auth/`)
...
Les profils d'authentification sont stockés dans `~/.spawnbot/auth.json` :
...
## Créer un nouveau fournisseur dans Spawnbot

Les fournisseurs Spawnbot sont implémentés en tant que packages Go sous `pkg/providers/`. Pour ajouter un nouveau fournisseur :
...
Si votre fournisseur nécessite OAuth ou une authentification spéciale, ajoutez un cas dans `cmd/spawnbot/internal/auth/helpers.go` :
...
- **Fichiers source :**
  ...
  - `pkg/auth/store.go` - Stockage des identifiants d'authentification (`~/.spawnbot/auth.json`)
  ...
  - `cmd/spawnbot/internal/auth/helpers.go` - Commandes CLI d'authentification
...
- Rafraîchir les jetons OAuth : `spawnbot auth login --provider antigravity`
...
- Vérifier le stockage du profil d'authentification : `~/.spawnbot/auth.json`
- Relancer `spawnbot auth login --provider antigravity`
