...
Communiquez avec votre Spawnbot via Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot ou MaixCam.
...
| **Pico**             | ⭐ Facile          | Canal protocole natif Spawnbot                        |                                                                                                                  |
...
Spawnbot conserve les définitions de commandes dans un registre partagé unique. Au démarrage, Telegram enregistre automatiquement les commandes bot prises en charge (par exemple `/start`, `/help`, `/show`, `/list`) afin que le menu de commandes et le comportement à l'exécution restent synchronisés.
...
Si l'enregistrement des commandes échoue (erreurs transitoires réseau/API), le canal démarre quand même et Spawnbot réessaie l'enregistrement en arrière-plan.
...
Spawnbot peut se connecter à WhatsApp de deux manières :
...
Si `session_store_path` est vide, la session est stockée dans `<workspace>/whatsapp/`. Lancez `spawnbot gateway` ; au premier lancement, scannez le code QR affiché dans le terminal avec WhatsApp → Appareils liés.
...
Spawnbot prend en charge la connexion à votre compte WeChat personnel via l'API officielle Tencent iLink.

**1. Connexion**

Lancez le flux de connexion interactif par QR code :
```bash
spawnbot auth weixin
```
...
4. Lancez `spawnbot gateway` et ouvrez QQ pour discuter avec votre bot
...
* Configurez comme indiqué ci-dessus et lancez `spawnbot gateway`
...
Spawnbot prend en charge trois types d'intégration WeCom :
...
Spawnbot se connecte à Feishu via le mode WebSocket/SDK — aucune URL webhook publique ni serveur de callback nécessaire.
...
OneBot est un protocole ouvert pour les bots QQ. Spawnbot se connecte à toute implémentation compatible OneBot v11 (par ex. [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
