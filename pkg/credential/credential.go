...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via SPAWNBOT_KEY_PASSPHRASE
...
//  2. SPAWNBOT_SSH_KEY_PATH env var
//  3. ~/.ssh/spawnbot_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "SPAWNBOT_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading SPAWNBOT_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "SPAWNBOT_SSH_KEY_PATH"

// spawnbotHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const spawnbotHome = "SPAWNBOT_HOME"

const (
	fileScheme = "file://"
	encScheme  = "enc://"
	hkdfInfo   = "spawnbot-credential-v1"
	saltLen    = 16
	nonceLen   = 12
	keyLen     = 32
)
...
//
// passphrase is required (SPAWNBOT_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// SPAWNBOT_SSH_KEY_PATH env var or ~/.ssh/spawnbot_ed25519.key.
...
//   - exact match with SPAWNBOT_SSH_KEY_PATH env var
//   - within the SPAWNBOT_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with SPAWNBOT_SSH_KEY_PATH.
	...
	// Within SPAWNBOT_HOME.
	if picoHome := os.Getenv(spawnbotHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="spawnbot-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set SPAWNBOT_SSH_KEY_PATH or place key at ~/.ssh/spawnbot_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (SPAWNBOT_SSH_KEY_PATH, SPAWNBOT_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. SPAWNBOT_SSH_KEY_PATH env var
//  3. ~/.ssh/spawnbot_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the spawnbot-specific SSH key path if it exists.
