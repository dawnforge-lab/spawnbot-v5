...
// with cf-mitigated: challenge triggers a retry using the honest spawnbot User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest spawnbot user agent
	if !strings.Contains(receivedUAs[1], "spawnbot") {
		t.Errorf("retry request UA = %q, want it to contain 'spawnbot'", receivedUAs[1])
	}
}
