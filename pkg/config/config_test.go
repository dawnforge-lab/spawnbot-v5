...
// SPAWNBOT_SSH_KEY_PATH to its path for the duration of the test. This is required
// whenever a test exercises encryption/decryption via credential.Encrypt or SaveConfig.
func mustSetupSSHKey(t *testing.T) {
	t.Helper()
	keyPath := filepath.Join(t.TempDir(), "spawnbot_ed25519.key")
	if err := credential.GenerateSSHKey(keyPath); err != nil {
		t.Fatalf("mustSetupSSHKey: %v", err)
	}
	t.Setenv("SPAWNBOT_SSH_KEY_PATH", keyPath)
}
  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "spawnbot-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".spawnbot", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithSpawnbotHome(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join("/custom/spawnbot/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with SPAWNBOT_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
...
`
	...
	t.Setenv("SPAWNBOT_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("SPAWNBOT_SSH_KEY_PATH", "")
...
// unchanged when SPAWNBOT_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("SPAWNBOT_KEY_PASSPHRASE", "")
	t.Setenv("SPAWNBOT_SSH_KEY_PATH", "")
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when SPAWNBOT_KEY_PASSPHRASE
...
func TestConfigLogLevelEmpty(t *testing.T) {
	...
	// When config omits log_level, the DefaultConfig value ("debug") is preserved.
	if cfg.Gateway.LogLevel != "debug" {
		t.Errorf("LogLevel = %q, want \"debug\"", cfg.Gateway.LogLevel)
	}
}
