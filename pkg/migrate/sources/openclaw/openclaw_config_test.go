...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.spawnbot/workspace" {
		t.Errorf("expected workspace '~/.spawnbot/workspace', got '%s'", workspace)
	}
...
func TestConvertToSpawnbot(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToSpawnbot("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.spawnbot/workspace" {
		t.Errorf("expected workspace '~/.spawnbot/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToSpawnbotWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToSpawnbot("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToSpawnbotWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToSpawnbotWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.spawnbot/workspace" {
		t.Errorf("expected workspace '~/.spawnbot/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
