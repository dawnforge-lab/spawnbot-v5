...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("spawnbot")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@spawnbot hello", want: true},
		{text: "hi @spawnbot:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bspawnbot\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@spawnbot:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@spawnbot:matrix.org hello", want: "hello"},
		{in: "@spawnbot, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@spawnbot:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@spawnbot:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@spawnbot:matrix.org">Spawnbot</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40spawnbot%3Amatrix.org">Spawnbot</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@spawnbot:matrix.test"), "")
	if err != nil {
