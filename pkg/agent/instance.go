...
type AgentInstance struct {
	...
	// TaskStore provides persistent task tracking for the agent.
...
func NewAgentInstance(
	...
	// embeddings provider is configured. Requires CGO for sqlite-vec.
	registerSemanticMemory(cfg, workspace, toolsRegistry)
...
func (ai *AgentInstance) CloneForHeartbeat(modelName string) *AgentInstance {
	...
	return &AgentInstance{
		...
		Tools: ai.Tools.CloneOnly(
			...
			"end_turn", "fire_event", "list_events",
		),
...
func resolveAgentWorkspace(agentCfg *config.AgentConfig, defaults *config.AgentDefaults) string {
	...
	// Use the configured default workspace (respects SPAWNBOT_HOME)
