...
func TestProcessHook_HelperProcess(t *testing.T) {
	if os.Getenv("SPAWNBOT_HOOK_HELPER") != "1" {
		return
	}
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"SPAWNBOT_HOOK_HELPER=1",
		"SPAWNBOT_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "SPAWNBOT_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("SPAWNBOT_HOOK_MODE")
	eventLog := os.Getenv("SPAWNBOT_HOOK_EVENT_LOG")
